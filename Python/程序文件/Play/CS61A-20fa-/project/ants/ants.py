...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(self,ContainerAnt) and self.can_contain(place.ant):
                ...
                place.ant = self   
            # note:this two instructions can't be reversed
            # otherwise it will cause inifite recursion loop          
    ...
    def remove_from(self, place):
        if place.ant is self:
            if self.name == 'Queen' and self.is_TrueQueen :
                return 
            else:
                place.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while cur_place != beehive :
            if  cur_place.bees and self.min_range <= counter <= self.max_range: 
    # Q3:or explicitly:len(cur_place.bees)>0  Q4:only take effects when counter is within range 
    ...
    def reduce_armor(self, amount):
        ...
        if armor <=0 :
            for bee in bees_copy:
                Ant.reduce_armor(bee,amount+self.damage)   
            # regard bee as list.remove(item),i.e. it find the exact same bee and
            # execute on it ,not by solution of indexing through a mutated list 
        else:               
            for bee in bees_copy:
                Ant.reduce_armor(bee,amount)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        if bee == None:
            ...
            Ant.reduce_armor(bee,bee.armor)
        ...
        if self.is_TrueQueen:   
            ...
            while cur_place != None:
                if cur_place.ant !=None:
                    if cur_place.ant.have_doubled_damage == False:
                        cur_place.ant.damage = cur_place.ant.damage * 2
                        cur_place.ant.have_doubled_damage = True
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        for bee in bees_copy:
            Ant.reduce_armor(bee,self.damage)
        # END Problem Optional 1
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        if self.contained_ant != None:  
    # without the if statement,the error 'Nonetype have no container_ant attribute'
    # will occur            
            self.contained_ant.action(gamestate)
        # END Problem Optional 3
############
...
def apply_status(status, bee, length):
    ...
    # because when the length runs out
    # we still need the old action
    # so we explicitly write it out
    # for later use (some kind of stroing temporarily)
    def NEW_ACTION(gamestate):
        nonlocal length
        if length > 0: 
            new_action(gamestate)
            length -= 1
        else: # when length go 0,we should reset the rev_direction to False
            if status == make_scare:
                bee.rev_direction = False
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 5
        insects_dict = {}
        LaserAnt_postion = self.place
        cur_position = LaserAnt_postion
        distance = 0
        while cur_position != beehive:
            if cur_position.ant != None and cur_position.ant != self:
                insects_dict[cur_position.ant] = distance
            if cur_position.bees != None:
                for bee in cur_position.bees:
                    insects_dict[bee] = distance
            cur_position = cur_position.entrance
            distance += 1            
        return insects_dict
        # END Problem Optional 5
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        goal_damage = self.damage - distance * 0.2 - 0.05 * self.insects_shot
        if goal_damage > 0:
            return goal_damage
        else:    
            return 0
