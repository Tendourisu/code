...
from optparse import Values  # Python 2 compatibility
...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    # first and rest are all valid Pair expressions(scheme lists)   
    first, rest = expr.first, expr.rest 
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
     #SPECIAL_FORMS is a dictionanry,so SPECIAL_FORMS[first] is a special form apply function
     # such as do_define_form function   
        return SPECIAL_FORMS[first](rest, env) 
    else:
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # here we don't have to validate the operator
        # because there is a validata function in the scheme_apply
        eval_operator = scheme_eval(first,env) #this later will be executed in scheme_eval atomic exprssion condition
        eval_operands = rest.map(lambda temp :scheme_eval(temp,env))
        #evaluate the operands : turn every operand into valid operand(e.g.numbers instead of symbols)
        #so we should contain the env(where the env takes effect)
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        is_find = False
        cur_env = self
        while not is_find and cur_env != None:
            if symbol in cur_env.bindings:
                is_find = True
                return cur_env.bindings[symbol]
    ...
    def make_child_frame(self, formals, vals):
        ...
        for i in range(len(formals)):
            ...
            value = vals.first
            child_frame.define(formal,value)
...
def do_define_form(expressions, env):
    ...
    note:the input expression is the scheme list behind the 'define' keyword
    >>> env = create_global_frame()
    ...
    target = expressions.first #the symbol which to be defined
    if scheme_symbolp(target):
        validate_form(expressions, 2, 2) # Checks that expressions is a list of length exactly 2
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        # note: expressions.rest cann't be the argument of the scheme_eval
        # because the expression.rest may be a nested scheme list(like Pair(valid_expression to be evaled,nil))
        # e.g. do_define_form(read_line("(x (+ 2 8))"), env) Pair(x, Pair(Pair('+',Pair....),nil))
        #                                                                 the true scheme expression to be evaled
        ...
        lambda_expression = Pair(formals,body)#actually this expression is kind of unnecessary.I use it just to satisfy the direction in the project file
        env.define(func_name,do_lambda_form(lambda_expression,env))  
        return func_name
        # END PROBLEM 9
    else:
